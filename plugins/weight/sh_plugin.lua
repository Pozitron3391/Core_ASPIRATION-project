...
if (CLIENT) then
	...
	function PLUGIN:PopulateItemTooltip(tooltip, item)
		...
		if !item.entity then
			if (carryinc) then
				ix.util.PropertyDesc2(tooltip, "Увеличение грузоподъемности: "..ix.weight.WeightString(carryinc, ix.option.Get("imperial", false)), Color(255, 255, 255), Material("vgui/ui/stalker/armorupgrades/carryweightinc.png"))
			end
		end

		if (weight) then
			ix.util.PropertyDesc3(tooltip, "Масса: "..ix.weight.WeightString(weight, ix.option.Get("imperial", false)), Color(255, 255, 255), Material("vgui/ui/stalker/weaponupgrades/weight.png"), 999)
		end
	end
end
