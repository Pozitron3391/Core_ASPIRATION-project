...
surface.CreateFont("def_pozi_26", {
    font = "Default",
    size = 26,
    antialias = false
})

surface.CreateFont("def_pozi_20", {
    font = "Default",
    size = 20,
    antialias = false
})

surface.CreateFont("def_pozi_16", {
    font = "Default",
    size = 16,
    antialias = false
})

surface.CreateFont("def_pozi_42", {
    font = "Default",
    size = 42,
    antialias = false
})
...
function PLUGIN:HUDPaint()
	...
	surface.DrawTexturedRectUV(ScrW()*0.05, ScrH()*0.895, math.Clamp(LocalPlayer():Health()/LocalPlayer():GetMaxHealth(), 0, 1)*ScrW()*0.15, ScrH()*0.008, 0, 0, math.Clamp(LocalPlayer():Health()/LocalPlayer():GetMaxHealth(), 0, 1), 1)

    surface.SetMaterial(Texture2)
	surface.SetDrawColor(Color(255, 255, 255, 255))
	surface.DrawTexturedRectUV(ScrW()*0.05, ScrH()*0.9074, math.Clamp(LocalPlayer():Armor()/LocalPlayer():GetMaxArmor(), 0, 1)*ScrW()*0.15, ScrH()*0.008, 0, 0, math.Clamp(LocalPlayer():Armor()/LocalPlayer():GetMaxArmor(), 0, 1), 1)
