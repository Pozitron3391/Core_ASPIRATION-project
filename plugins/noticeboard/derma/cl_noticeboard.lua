local PANEL = {}
	function PANEL:Init()
		...
		title = self:Add("DLabel")
			...
			title:SetText("Доска объявлений")
	...
	function PANEL:loadText(index)
		s2:AlphaTo(0, 0.3, 0, function()
			...
			removeText = s2:Add("DButton")
				...
				removeText:SetText("Удалить")
				removeText:SetDisabled(false) --WAS TRUE
				removeText.DoClick = function()
					Derma_Query("Вы уверены, что хотите удалить это уведомление?", "Подтверждение", "Да", function()
			...
			editText = s2:Add("DButton")
				...
				editText:SetText("Редактировать")
	...
	function PANEL:editText(index)
		s2:AlphaTo(0, 0.3, 0, function()
			...
			noticeEdit = s2:Add("DTextEntry")
				...
				noticeEdit:SetFont("def_pozi_16")
				if(notices[index]) then
	...
	function PANEL:populate(data)
		...
		newNotice = s1:Add("DButton")
			...
			newNotice:SetText("Новое объявление")
			newNotice.DoClick = function()
				...
				for i, v in ipairs(notices) do
					...
					if noticeCheck >= 5 then 
							return LocalPlayer():Notify("Вы можете иметь только 5 уведомлений одновременно!")
						end
					end
				end

				Derma_StringRequest("Название объявления", "Как бы вы хотели озаглавить свое объявление?", "", function(titleLabel)
					if (string.len(titleLabel) < 6) then LocalPlayer():Notify("Название слишком короткое!") return end
					if (string.len(titleLabel) > 40) then LocalPlayer():Notify("Название слишком длинное!") return end
