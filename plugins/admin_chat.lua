...
ix.chat.Register("adminchat", {
	...
	OnChatAdd = function(self, speaker, text)
		...
		--local icon = ulx.IsAdmin:GetRank(ulx.player:GetRank(speaker)).texture or "icon16/user.png"


		icon = Material(hook.Run("GetPlayerIcon", speaker) or "icon16/user.png")

		

		if (CAMI.PlayerHasAccess(LocalPlayer(), "Helix - Admin Chat", nil) and CAMI.PlayerHasAccess(speaker, "Helix - Admin Chat", nil)) then

			chat.AddText(icon, Color(255, 215, 0), "[А] ", Color(128, 0, 255, 255), ( "("..speaker:SteamID()..") " or "" )..speaker:SteamName(), ": ", Color(255, 255, 255), text)
