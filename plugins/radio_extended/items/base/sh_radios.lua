...
function ITEM:GetDescription()
	...
	if enabled then
		if self.hybrid then
			ret = string.format("%s\nРаботает в %s режиме.", ret, self:GetData("duplex",self.duplex) and "duplex" or "simplex")
		end
		...
		ret = ret.."\nРация настроена на канал "..self:GetData("channel","1")..( (adStr != defCh) and ", известный как "..adStr.."." or ".")
	end
