...
PLUGIN.name = "Рация"
PLUGIN.author = "faust", "Pozitron#2407"
...
if (CLIENT) then
	...
	netstream.Hook("Channel", function(names)
		...
		local quer = Derma_Query("Щелкните левой кнопкой мыши, чтобы выбрать канал\nЩелкните правой кнопкой мыши, чтобы переименовать канал", "Выбор канала",
		...
		function ch1panel:DoRightClick()
			Derma_StringRequest("Название канала", "Каким бы вы хотели видеть новое название?\nНажмите пробел, чтобы сбросить имя канала по умолчанию", names[1], function(text)
				if text == " " then
					setTheName("1","Ганза")
				else
					setTheName("1",text)
		...
		function ch2panel:DoRightClick()
			Derma_StringRequest("Название канала", "Каким бы вы хотели видеть новое название?\nНажмите пробел, чтобы сбросить имя канала по умолчанию", names[2], function(text)
				if text == " " then
					setTheName("2","Рейх")
				else
					setTheName("2",text)
		...
		function ch3panel:DoRightClick()
			Derma_StringRequest("Название канала", "Каким бы вы хотели видеть новое название?\nНажмите пробел, чтобы сбросить имя канала по умолчанию", names[3], function(text)
				if text == " " then
					setTheName("3","Красные")
				else
					setTheName("3",text)
				end
				quer:Close()
		...
		function ch4panel:DoRightClick()
			Derma_StringRequest("Название канала", "Каким бы вы хотели видеть новое название?\nНажмите пробел, чтобы сбросить имя канала по умолчанию", names[4], function(text)
				if text == " " then
					setTheName("4","Спарта")
				else
					setTheName("4",text)
...
ix.config.Add("radioColor",Color(164,224,91), "Цвет по умолчанию для радиочата.", nil, {category = "Рация"})
--ix.config.Add("radioYellColor",Color(164+30,224+30,91+30), "Цвет по умолчанию для криков в радиочате.", nil, {category = "Рация"})
ix.config.Add("longRangeColor", Color(255,139,82), "Цвет по умолчанию для дальнего радиочата.", nil, {category = "Рация"})
ix.config.Add("walkieTalkieColor", Color(91,182,224), "Цвет по умолчанию для радиочата рации.", nil, {category = "Рация"})
--ix.config.Add("longRangeYellColor",Color(255,139+30,82+30), "Цвет по умолчанию для криков в дальнем радиочате.", nil, {category = "Рация"})

ix.config.Add("radioFreqColor",Color(175,175,175), "Цвет по умолчанию для радиочастот.", nil, {category = "Рация"})
ix.config.Add("activeFreqColor", Color(245,245,245), "Цвет по умолчанию для радиочастот дальнего действия.", nil, {category = "Рация"})

ix.config.Add("radioYellBig", true, "Использовать ли шрифт большего размера для криков на радио.", nil, {
	category = "Рация"
})
ix.config.Add("radioWhisperSmall", true, "Использовать ли меньший размер шрифта для шепота по радио.", nil, {
	category = "Рация"
})

ix.config.Add("radioDecayModel", 3, "Модель, используемая для расчета того, как зашифрованные радиосообщения становятся на расстоянии..\n\n"..
	"(1) Квадратичный (x^2)\nХорошая ближняя дистанция, плохая средняя дистанция\n\n"..
	"(2) Логарифмический (x^(1/3) Log10[2]/Log10[2/x])\nХудший в целом, но самый плавный/самый предсказуемый спад\n\n"..
	"(3) Гибридный (0.5(x + x^8))\nНа близком расстоянии хуже, чем на квадратичном, на дальнем лучше, чем на логарифмическом\n\n"..
	"(4) Самый низкий (0.5(x^2 + x^9))\nЛучший в целом, с приблизительно квадратичным затуханием на большом расстоянии", nil, {
	data = {min = 1, max = 4},
	category = "Рация"
})
...
ix.config.Add("radioRangeMult", 100, "Максимальная дальность радиосвязи = дальность чата IC * mult", nil, {
	data = {min = 1, max = 175},
	category = "Рация"
})

ix.config.Add("repeaterMult", 2, "Максимальная дальность действия ретранслятора = дальность радиосвязи * mult", nil, {
	data = {min = 1, max = 20},
	category = "Рация"
})

ix.config.Add("longrangeMult", 2, "Максимальная дальность радиосвязи = дальность радиосвязи * mult", nil, {
	data = {min = 1, max = 10},
	category = "Рация"
})

ix.config.Add("walkieMult", 0.3, "Максимальная дальность действия рации = дальность радиосвязи * mult\n(всегда короче радиуса действия радио)", nil, {
	data = {min = 0, max = 1, decimals=1},
	category = "Рация"
})


ix.config.Add("garbleRadio", true, "Искажается ли радиопереговоры на больших расстояниях.", nil, {
	category = "Рация"
})

ix.config.Add("garbleOffset", 0, "Необработанный плюс-минус модификатор к дроби искажения, независимо от того, что.\nПочти всегда лучше настроить другие параметры перед этим!", nil, {
	data = {min = -100, max = 100},
	category = "Рация"
})

-- ix.config.Add("garbleMaxFrac", 60, "Максимальная доля скремблирования на максимальном расстоянии радиосвязи. Чем ниже, тем меньше искажений на всех расстояниях.", nil, {
	-- data = {min = 0, max = 100},
	-- category = "Рация"
-- })

ix.config.Add("enableCallsigns", true, "Разрешены ли позывные.", nil, {
	category = "Рация"
})

ix.config.Add("anonymousRadioNames", false, "Если true, имена игроков не будут отображаться при использовании радио, и"..
	"вместо этого будет заменено на «Кто-то», «Кто-то», «Голос» или «Человек». "..
	"Позывные становятся единственным способом однозначно идентифицировать кого-либо на радио.", nil, {
	--data = {min = 1, max = 2},
	category = "Рация"
})

ix.config.Add("allowBroadcast", true, "Разрешено ли вещание на всех каналах для любого радио.", nil, {
	--data = {min = 1, max = 2},
	category = "Рация"
})
...
function PLUGIN:OverwriteClasses()
	...
	do
		local CLASS = {}
		CLASS.color = ix.config.Get("255, 0, 175") -- ix.chat.classes.ic.color Color(255, 255, 175)
		...
		function COMMAND:OnRun(client, chan)
			...
			if (self:TableLength(radios) < 1) then
				...
				if itemTable then
					...
					client:Notify(string.format("Вы переключили канал связи на %s.", chan))
				else
					client:Notify("У вас нет активной рации.")
				end
			end
		end
...
-- заполняет метки на экране состояния
