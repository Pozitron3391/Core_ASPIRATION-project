...
ITEM.description = "Небольшой прибор для измерения колебаний давления, а также газообразных примесей. Во внутренней памяти можно сохранить пару GPS-позиций, чтобы исследовательская группа знала, куда идти."
...
ITEM.functions.zCheck = {
	name = "Проверить назначенные баллы", -- Check Assigned Points 
	icon = "icon16/stalker/repair.png",
	OnRun = function(item)
		if (item:GetData("map", "") == game.GetMap()) then
			...
			item.player:Notify("Неподходящая область")
		end
