...
ix.config.Add("reviveOn", true, "Включить?", nil, {
	category = "Revive"
})

ix.config.Add("reviveRessurrectionTime", 2, "Сколько времени нужно, чтобы оживить человека.", nil, {
	data = {min = 1, max = 60},
	category = "Revive"
})
		...
		if (LocalPlayer():GetChar()) then
			for k, v in pairs(ents.FindByClass("prop_ragdoll")) do
				if IsValid(v) and v.isDeadBody then
					...
					draw.SimpleText("Осталось времени: "..math.Round(v:GetNWFloat("Time") - CurTime()), "ReviveText", Pos.x, Pos.y - 20, Color(249, 255, 239), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
				end
			end
		end
	end)
